...
<!-- <main class="mt-8" class="flex-grow p-8 transition-all duration-300 ">
    ...
    <span class="text-gray-800 overview">Completed</span>
</p>
...
<div class="w-full sm:w-full lg:w-/3 mb-4 sm:mb-0">
    <div class="statushead bg-green-500 rounded-t-lg shadow-md p-2">
        <h2 class="textcolor text-lg font-semibold text-white flex items-center">
        <i class='iconstatus bx bx-check-circle text-lg text-white mr-2'></i>  Completed
            <span class="countstatus bg-white text-gray-700 rounded-full ml-2 px-2.5 py-1.5 text-xs"><?php echo $taskCounts['completed']; ?></span>
        </h2>
    </div>
    <div class="statuscontainer bg-green-100 p-2 pt-5">
        <?php
        require('config.php');

        $query = "SELECT id, title, due_date, status FROM task_list WHERE status = 'completed'";
...
echo "<div class='progress' style='height: 100%; width: 100%; background-color: #48bb78; border-radius: 5px;'></div>";
echo "<p class=' contenttext text-xs text-gray-500 mt-1'>100%</p>"; 
echo "</div>";

                        echo "<div class='relative mt-7'>";
                        echo "<select name='status' class='statuscontainer selecttext bg-gray-200  rounded-md px-3 py-2 bg-white appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-700 task-status' data-task-id='" . $taskId . "'>";
                        ...
                        echo "<i class='fas fa-circle text-green-500'></i>";
                        echo "</div>";
                        echo "</div>";
                        echo "<div class='mt-4'>";
