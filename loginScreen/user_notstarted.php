...
<!-- <main class="mt-8" class="flex-grow p-8 transition-all duration-300 ">
    ...
    <span class="text-gray-800 overview">Not started</span>
</p>
...
<div class="w-full sm:w-full lg:w-/3 mb-4 sm:mb-0">
    <div class="statushead bg-red-500 rounded-t-lg shadow-md p-2">
        <h2 class="textcolor text-lg font-semibold text-white flex items-center">
            <i class='iconstatus bx bx-time-five text-lg text-white mr-2'></i> Not started
            <span class="countstatus bg-white text-gray-700 rounded-full ml-2 px-2.5 py-1.5 text-xs"><?php echo $taskCounts['not_started']; ?></span>
        </h2>
    </div>
    <div class="statuscontainer bg-red-100 p-2 pt-5">
        <?php
        require('config.php');
        $query = "SELECT id, title, due_date, status FROM task_list WHERE status = 'Not started'";
            ...
            if ($result->num_rows > 0) {
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<div class='progress' style='height: 100%; width: 0%; background-color: #ff6347; border-radius: 5px;'></div>";
                        echo "<p class='contenttext  text-xs text-gray-500 mt-1'>0%</p>"; 
                        echo "</div>";
                        ...
                        echo "<i class='fas fa-circle text-red-500'></i>";
                        echo "</div>";
                        echo "</div>";
                        echo "<div class='mt-4'>";
                        ...
                        // echo "<button class='textcolor text-blue-500 font-bold  rounded-md ' onclick='saveComment($taskId)'><i class='statuscontainer textcolor bg-blue-300 p-2 rounded-lg text-blue-500 fas fa-save mr-1'></i></button>";
                        // echo "<button class='textcolor text-red-500 font-bold px-3 py-1 rounded-md' onclick='deleteComment($taskId)'><i class='statuscontainer textcolor bg-red-300 p-2 rounded-lg text-red-500 fas fa-trash-alt mr-1'></i></button>";
                        // echo "</div>";                        
                        echo "</div>";
...
<!-- <div class="w-full sm:w-1/3 px-2 mb-4 sm:mb-0">
    <div class="bg-yellow-500 rounded-t-lg shadow-md p-2">
        <h2 class="text-lg font-semibold text-white flex items-center">
            <i class='bx bx-loader-circle text-lg text-white mr-2'></i> In progress
            <span class="bg-white text-gray-700 rounded-full ml-2 px-2.5 py-1.5 text-xs"><?php echo $taskCounts['in_progress']; ?></span>
        </h2>
    </div>
    <div class="bg-yellow-200 p-2 flex justify-center">
    
    </div>
</div>

<div class="w-full sm:w-1/3 px-2">
    <div class="bg-green-500 rounded-t-lg shadow-md p-2">
        <h2 class="text-lg font-semibold text-white flex items-center">
            <i class='bx bx-check-circle text-lg text-white mr-2'></i> Completed
            <span class="bg-white text-gray-700 rounded-full ml-2 px-2.5 py-1.5 text-xs"><?php echo $taskCounts['completed']; ?></span>
        </h2>
    </div>
    <div class="bg-green-200 p-2 flex justify-center">
    <div class="bg-white rounded-lg p-4" style="width: 80%;">
...
function sortTable(order) {
    ...
    rows.sort((rowA, rowB) => {
        const cellA = rowA.children[2].textContent.trim(); // Change index according to your structure
        const cellB = rowB.children[2].textContent.trim(); // Change index according to your structure
