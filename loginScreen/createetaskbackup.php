...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check if the task already exists for any of the assigned users
    ...
    if ($check_result && mysqli_num_rows($check_result) > 0) {
        ...
        // Insert the task into task_list table
        $insert_sql = "INSERT INTO task_list (title, description, due_date, priority) VALUES (?, ?, ?, ?)";
        $stmt = mysqli_prepare($connection, $insert_sql);
        mysqli_stmt_bind_param($stmt, "ssss", $title, $description, $due_date, $priority);
        $success = mysqli_stmt_execute($stmt);

        if ($success) {
            $task_id = mysqli_insert_id($connection); // Get the ID of the inserted task

            // Assign the task to multiple users in the multiple_users table
            foreach ($assigned_to as $user_id) {
                $insert_assignment_sql = "INSERT INTO multiple_users (task_id, user_id) VALUES (?, ?)";
...
?>
    ...
    <img src="undraw_add_files_re_v09g.svg" alt="Welcome Illustration" class="animate-bounce h-8 w-auto sm:h-12 transition-transform duration-500 ease-in-out transform hover:scale-110">
        ...
        <select name="assigned_to[]" class="border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline required" multiple>
    <?php
    include('config.php');
